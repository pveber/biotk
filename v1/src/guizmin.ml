...
and _ kind =
  ...
  | Val7 : 'b pipeline * 'c pipeline * 'd pipeline * 'e pipeline * 'f pipeline * 'g pipeline * 'h pipeline * (env -> 'b -> 'c -> 'd -> 'e -> 'f -> 'g -> 'h -> 'a) -> 'a kind
...
let v7 id params x y z w r s t f = {
  id ; params ;
  hash = make_hash id params `value [ x.hash ; y.hash ; z.hash ; w.hash ; r.hash ; s.hash ; t.hash ] ;
  kind = Val7 (x, y, z, w, r, s, t, f)
}
...
let path : type s. base:string -> s pipeline -> string = fun ~base x ->
  ...
  | Dir_input path -> path
  | Val0 _ | Val1 _ | Val2 _ | Val3 _ | Val4 _ | Val5 _ | Val6 _ | Val7 _ -> cache_path
  | File0 _ -> cache_path
  | File1 _ -> cache_path
  | File2 _ -> cache_path
  | File3 _ -> cache_path
...
let with_env ?(np = 1) ?(mem = 100) base x ~f =
  ...
  let env = {
    ...
    with_temp_file = fun f -> Guizmin_utils.with_temp_file ~in_dir:(tmp_dir base) ~f
  }
  in
  let r = f env in
...
let rec fold : type a. 's update -> 's -> a pipeline -> 's = fun up init x ->
  if up.guard x then
    ...
    | Val7 (x1,x2,x3,x4,x5,x6,x7,_) -> up.f (fold up (fold up (fold up (fold up (fold up (fold up (fold up init x1) x2) x3) x4) x5) x6) x7) x
...
let fold_deps : type a. 's update -> 's -> a pipeline -> 's = fun up init x ->
  if up.guard x then
    ...
    | Val7 (x1,x2,x3,x4,x5,x6,x7,_) -> up.f (up.f (up.f (up.f (up.f (up.f (up.f init x1) x2) x3) x4) x5) x6) x7
    | File_input _ -> init
    | File0 _ -> init
...
let rec built : type a. base:string -> a pipeline -> bool = fun ~base x ->
  ...
  | Val0 _ | Val1 _ | Val2 _ | Val3 _ | Val4 _ | Val5 _ | Val6 _ | Val7 _ ->
...
let rec unsafe_eval : type a. base:string -> a pipeline -> a = fun ~base x ->
  match x.kind with
  | Val0 _ | Val1 _ | Val2 _ | Val3 _ | Val4 _ | Val5 _ | Val6 _ | Val7 _ -> load_value (path base x)
...
let exec : type a. a pipeline -> env -> unit = fun x env ->
  ...
  | Val7 (x1, x2, x3, x4, x5, x6, x7, f) -> val_wrap x (fun env -> f env (eval x1) (eval x2) (eval x3) (eval x4) (eval x5) (eval x6) (eval x7))
...
let load_value : 'a value file_path -> 'a =
  function (File fn) -> Guizmin_utils.load fn
