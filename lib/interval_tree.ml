...
let rec to_seq t =
  ...
  | Node n ->
    Seq.append
      (to_seq n.left)
      (Seq.append (Seq.return (node_contents n)) (to_seq n.right))

let rec to_backwards_seq t =
  match t with
  | Empty -> Seq.empty
  | Node n ->
    Seq.append
      (to_backwards_seq n.right)
      (Seq.append
         (Seq.return (node_contents n))
         (to_backwards_seq n.left))
...
let find_closest t lo hi = match find_closest_aux lo hi t with
    Some (n,d) ->
...
let find_intersecting_elem t lo hi =
